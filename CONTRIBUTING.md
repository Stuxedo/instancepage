...
- General contact uses `hello@stuxedo.com`; legal-page contact uses `legal@stuxedo.com`. The dynamic `ZAK@<region>.stuxedo.net` mailbox on the instance card is separate — it's derived per-instance from the referrer, not a static company address.
